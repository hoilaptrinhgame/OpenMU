...
using MUnique.OpenMU.Launcher.Helpers.Torrent.BEncoding;
using MUnique.OpenMU.Launcher.Helpers.Torrent.Exceptions;
...
namespace MUnique.OpenMU.Launcher.Helpers.Torrent
{
    ...
    public sealed class TorrentInfo
    {
        ...
        private TorrentInfo(BEncodedDictionary dictionary, string infoHash, long pieceLength, IEnumerable<string> piecesHashValues, bool isPrivate, IEnumerable<Uri> announceList,
            DateTime? creationDate, string comment, string createdBy, Encoding encoding, IEnumerable<TorrentFileInfo> files)
        {
        ...
        /// </value>
        public IEnumerable<Uri> AnnounceList { get; }
        ...
        ///     The piece hashes.
        /// </value>
        public IEnumerable<string> PieceHashes { get; }
                        ...
                        foreach (var item in info[filesKey].As<BEncodedList>())
                                ...
                                item.As<BEncodedDictionary>()[lengthKey] is BEncodedNumber)
                            {
                                filePath = Path.Combine(tmpString,
                                    Path.Combine(item.As<BEncodedDictionary>()[pathKey].As<BEncodedList>().Select(x => x.As<BEncodedString>().Text).ToArray()));
